...
//                    @HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds", value = "5000"),    //执行超时时间|default:1000
                    ...
                    @HystrixProperty(name = "fallback.isolation.semaphore.maxConcurrentRequests", value = "10"),   // 设置调用线程产生的HystrixCommand.getFallback()方法的允许最大请求数目。|default: 10
            },
...
//                    @HystrixProperty(name = "queueSizeRejectionThreshold", value = "15"),   //队满拒绝服务阈值|default:5|此值生效优先于队满, maxQueueSize设置为-1，该属性不可用。
    ...
    public BaseResp sendTimeoutHystrix(MessageReq messageReq){
        return iMessageFacade.sendTimeout(messageReq, 5000L);
    }

    public BaseResp sendTimeoutNoHystrix(MessageReq messageReq){
        return iMessageFacade.sendTimeout(messageReq, 5000L);
    }
    ...
    public BaseResp sendException(MessageReq messageReq){
        ...
        if (atomicInteger.get() % 3 == 1) return BaseResp.getInstance(LocalTime.now());
